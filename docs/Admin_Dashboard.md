...
<div class="hero-admin">
    ...
    <p id="fecha-dinamica">Hoy es [fecha en español]</p>
</div>
```

**JavaScript para fecha dinámica:**
```javascript
function actualizarFecha() {
    const opciones = { year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' };
    const hoy = new Date();
    const fechaFormato = new Intl.DateTimeFormat('es-ES', opciones).format(hoy);
    document.getElementById('fecha-dinamica').textContent = `Hoy es ${fechaFormato}`;
}
